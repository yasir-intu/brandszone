...
@section('content')
    ...
    <li class="breadcrumb-item"><a href="{{url('admin/recycle/contacts')}}">Recyle Messages</a></li>
    @endcomponent
                                ...
                                <tbody>
                                    @foreach($contactmessages as $key=>$contactMessage)
                                        <tr class="{{$contactMessage->status == 0 ? 'unreaded' : ''}}">
                                            ...
                                            <td>
                                                <a href="{{url('admin/recycle/contacts/'. $contactMessage->id)}}"><button class="btn btn-info btn-circle btn-sm"><i class="mdi mdi-eye"></i></button></a>
                                                <button data-url="{{url('admin/recycle/contacts/'. $contactMessage->id)}}" class="btn-restore btn btn-success btn-circle btn-sm" data-toggle="modal" data-target="#restoreModal"><i class="mdi mdi-recycle"></i></button>
                                                <button data-url="{{url('admin/recycle/contacts/'.$contactMessage->id)}}" class="btn-delete btn btn-danger btn-circle btn-sm" data-toggle="modal" data-target="#deleteModal"><i class="mdi mdi-delete"></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
